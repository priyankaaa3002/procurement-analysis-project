-- Wraps each analysis query as a view, so Power BI (or anything else) can
-- pull a clean, pre-shaped result set without re-writing the SQL.
...
CREATE OR REPLACE VIEW v_abc_pareto AS
...
ranked AS (
    SELECT *, SUM(spend) OVER (ORDER BY spend DESC) / SUM(spend) OVER () AS cum_pct
    FROM supplier_spend
)
...
CREATE OR REPLACE VIEW v_supplier_otif AS
WITH delivery_summary AS (
    SELECT po_id, MAX(delivery_date) AS final_delivery_date,
           SUM(delivered_qty) AS total_delivered,
           BOOL_AND(quality_status = 'Accepted') AS all_accepted
    FROM deliveries GROUP BY po_id
)
...
CREATE OR REPLACE VIEW v_ppv AS
...
CREATE OR REPLACE VIEW v_maverick_spend AS
SELECT business_unit, COUNT(*) AS maverick_po_count, SUM(ordered_amount) AS maverick_spend,
       ROUND(100.0 * SUM(ordered_amount) / (SELECT SUM(ordered_amount) FROM purchase_orders), 1) AS pct_of_total_spend
FROM purchase_orders
WHERE contract_id IS NULL
GROUP BY business_unit;

CREATE OR REPLACE VIEW v_savings_tracking AS
...
CREATE OR REPLACE VIEW v_kraljic_segmentation AS
WITH cat_spend AS (
...
CREATE OR REPLACE VIEW v_dpo AS
...
CREATE OR REPLACE VIEW v_should_cost AS
...
CREATE OR REPLACE VIEW v_demand_forecast_input AS
...
CREATE OR REPLACE VIEW v_split_po_candidates AS
